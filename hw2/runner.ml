...
let result_printer (e : (string, string) result) : string =
  match e with Error v -> sprintf "Error: %s\n" v | Ok v -> v
...
let test_run (program_str : string) (outfile : string) (expected : string)
...
let test_err (program_str : string) (outfile : string) (errmsg : string)
    (test_ctxt : OUnit2.test_ctxt) : unit =
  let result =
    try
      let full_outfile = "output/" ^ outfile in
