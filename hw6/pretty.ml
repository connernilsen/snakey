...
and string_of_expr_with (print_a : 'a -> string) (e : 'a expr) : string =
  ...
  | EApp(funname, args, call_type, a) ->
    sprintf "(%s%s(%s))%s" (string_of_call_type call_type) funname (ExtString.String.join ", " (List.map string_of_expr args)) (print_a a)
...
and string_of_cexpr_with (print_a : 'a -> string) (c : 'a cexpr) : string =
  ...
  | CApp(funname, args, call_type, a) ->
    sprintf "(%s%s(%s))%s" (string_of_call_type call_type) funname (ExtString.String.join ", " (List.map string_of_immexpr args)) (print_a a)
