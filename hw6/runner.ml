...
let test_run ?args:(args=[]) ?std_input:(std_input="") program_str outfile expected ?cmp:(cmp=(=)) test_ctxt =
...
let test_run_valgrind ?args:(args=[]) ?std_input:(std_input="") program_str outfile expected ?cmp:(cmp=(=))  test_ctxt =
  let full_outfile = "output/" ^ outfile in
  let result =
    try
      let program = parse_string outfile program_str in
...
let input_file_test_suite () =
  let safe_readdir dir ext =
    ...
    with _ -> [] in
  "input-file-suite">:::[
    "do_pass"  >:::(List.map (fun f -> f>::test_does_run f)   (safe_readdir "input/do_pass"   ".egg"));
    "do_err"   >:::(List.map (fun f -> f>::test_does_err f)   (safe_readdir "input/do_err"    ".egg"));
    "dont_pass">:::(List.map (fun f -> f>::test_doesnt_run f) (safe_readdir "input/dont_pass" ".egg"));
    "dont_err" >:::(List.map (fun f -> f>::test_doesnt_err f) (safe_readdir "input/dont_err"  ".egg"))
  ]
