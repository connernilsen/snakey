...
{
  printf("ESP: %#018lx\t==>  ", (uint64_t)esp);
  fflush(stdout);
  printHelp(stdout, *esp);
  fflush(stdout);
  printf("\nEBP: %#018lx\t==>  ", (uint64_t)ebp);
  fflush(stdout);
  printHelp(stdout, *ebp);
  fflush(stdout);
  printf("\n(difference: %ld)\n", (uint64_t)(esp - ebp));
  fflush(stdout);
...
void error(uint64_t code, SNAKEVAL val)
{
  ...
  case ERR_ARITH_NOT_NUM:
    ...
    break;
  case ERR_NOT_BOOL:
    fprintf(stderr, "expected a boolean, got ");
    printHelp(stderr, val);
    break;
  case ERR_DESTRUCTURE_INVALID_LEN:
    fprintf(stderr, "unable to destructure tuple with incorrect length ");
    printHelp(stderr, val);
    break;
  case ERR_OVERFLOW:
    fprintf(stderr, "overflow occurred for arithmetic operation, got ");
    printHelp(stderr, val);
    break;
  case ERR_GET_NOT_TUPLE:
    fprintf(stderr, "tuple access expected tuple, got ");
    printHelp(stderr, val);
    break;
  case ERR_GET_LOW_INDEX:
    fprintf(stderr, "index too small to get, got ");
    printHelp(stderr, val);
    break;
  case ERR_GET_HIGH_INDEX:
    fprintf(stderr, "index too large to get, got ");
    printHelp(stderr, val);
    break;
  case ERR_GET_NOT_NUM:
    fprintf(stderr, "get expected numeric index, got ");
    printHelp(stderr, val);
    break;
  case ERR_NIL_DEREF:
  ...
  case ERR_SET_NOT_NUM:
    fprintf(stderr, "set expected numeric index, got ");
    printHelp(stderr, val);
    break;
  case ERR_CALL_NOT_CLOSURE:
    fprintf(stderr, "tried to call a non-function value: ");
    printHelp(stderr, val);
    break;
  case ERR_CALL_ARITY_ERR:
    ...
    break;
  default:
    fprintf(stderr, "Unknown error code: %ld, val: ", code);
    printHelp(stderr, val);
  }
  free(HEAP);
  exit(code);
